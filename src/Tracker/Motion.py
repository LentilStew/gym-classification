...
class Motion:
    def __init__(self, file_name: str | None = None, tracker_settings: None | dict = None):
        ...
        self.settings_file_descriptor: BufferedRandom | None = None
    ...
    def as_linear(self) -> Tuple[np.ndarray,np.ndarray,np.ndarray,np.ndarray,np.ndarray,np.ndarray]:
        with self.settings_lock:
            stacked_acc = np.vstack(
                [packet.acc_readings for packet in self.packets])
            stacked_gyro = np.vstack(
                [packet.gyro_readings for packet in self.packets])

            x_acc, y_acc, z_acc = np.hsplit(stacked_acc, 3)
            x_gyro, y_gyro, z_gyro = np.hsplit(stacked_gyro, 3)

            return (x_acc.flatten(), y_acc.flatten(), z_acc.flatten(), x_gyro.flatten(), y_gyro.flatten(), z_gyro.flatten())
    ...
    def as_packet(self):
        ...
        for packet in packets:
            final_packet["streams"].append(packet.tolist())
    ...
    def open(self):
        with self.settings_lock:
            if isinstance( self.settings_file_name,str):
                self.settings_file_descriptor = open(self.settings_file_name, "+ab")
                self._is_open = True
